...
| Reusable atoms/molecules: inspecting `src/components/atoms/` (shadcn/Radix) and `src/components/molecules/` before authoring raw markup or ad-hoc primitives | [reuse-first](../reuse-first/SKILL.md) and [ui-styling](../ui-styling/SKILL.md) |
...
- [ ] **Atomic Primitives**: Generic components (Button, Table, Input, Dialog, Tooltip) reside in `src/components/atoms/` built on shadcn/Radix. Higher-level components reuse atoms and molecules.
