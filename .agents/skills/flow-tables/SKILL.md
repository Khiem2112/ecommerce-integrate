...
### Table Row Action Density

Apply the general text-button, icon-button, and overflow-menu decision rules in
[web-rules](../ui-ux-pro-max/rules/web-rules.md). For table rows specifically:

- Treat opening the row or its identity link as the primary action.
- Render at most two or three familiar secondary row actions as
  `Button size="icon"` controls with a Tooltip, localized `aria-label`, and an
  `aria-hidden="true"` icon. Do not place multiple full-width text buttons in a
  table cell.
- Put additional, infrequent, or destructive actions in an overflow menu.
  Destructive actions still require the recovery or confirmation behavior from
  [flow-errors](../flow-errors/SKILL.md).
- Reserve the action-area width so hover-revealed controls do not reflow the
  row. Keep the overflow trigger visible on touch devices.

```tsx
// Standard table row action pattern
