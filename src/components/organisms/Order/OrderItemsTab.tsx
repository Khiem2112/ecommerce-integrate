...
export function OrderItemsTab({
  ...
  const t = useTranslations('orders.itemsTab');
      ...
      <div className="flex items-center justify-between">
        <div>
          <h4 className="text-sm font-semibold text-foreground">
            {t('title', { count: order.items.length })}
          </h4>
        ...
        <Table className="min-w-[800px]">
          <TableHeader className="sticky top-0 z-10 bg-surface-lifted border-b border-hairline">
            <TableRow>
              ...
              <TableHead className="w-32 whitespace-nowrap">{t('sku')}</TableHead>
              <TableHead className="whitespace-nowrap">{t('name')}</TableHead>
              <TableHead className="w-32 whitespace-nowrap">{t('category')}</TableHead>
              <TableHead className="w-20 text-center whitespace-nowrap">{t('quantity')}</TableHead>
              <TableHead className="w-28 text-right whitespace-nowrap">{t('unitPrice')}</TableHead>
              <TableHead className="w-24 text-right whitespace-nowrap">{t('discount')}</TableHead>
              <TableHead className="w-32 text-right whitespace-nowrap">{t('lineTotal')}</TableHead>
              <TableHead className="w-16 text-right whitespace-nowrap">{t('delete')}</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
                  ...
                  <TableRow key={item.id}>
                    ...
                    <TableCell className="text-right">
                      <IconButton
                        variant="ghost"
                        size="sm"
                        ariaLabel={t('deleteItemAria')}
                        onClick={() => onDeleteItem(item.id)}
                        disabled={order.items.length <= 1}
                        className="text-muted hover:text-semantic-error disabled:opacity-50"
                        tooltip={order.items.length <= 1 ? t('minItemsTooltip') : t('deleteItemTooltip')}
                      >
                        <svg aria-hidden="true" className="size-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
