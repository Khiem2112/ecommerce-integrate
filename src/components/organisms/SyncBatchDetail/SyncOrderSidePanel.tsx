...
export function SyncOrderSidePanel({ batchId, selectedId, onSelect }: SyncOrderSidePanelProps) {
  ...
  return (
    <aside className="hidden w-70 shrink-0 overflow-hidden rounded-2xl border border-hairline bg-surface-card shadow-card xl:flex xl:flex-col" aria-label={t('orderNavigationAria')}>
      <div className="border-b border-hairline p-3">
        <h2 className="text-xs font-semibold text-foreground">{t('processedOrdersTitle')}</h2>
        <p className="mb-2 mt-0.5 text-[10px] text-muted">{t('processedOrdersDesc')}</p>
        <label htmlFor="sync-side-search" className="sr-only">{t('searchOrderAria')}</label>
        <Input id="sync-side-search" size="sm" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t('searchOrderPlaceholder')} />
      </div>
      <div className="max-h-[calc(100dvh-240px)] flex-1 overflow-y-auto custom-scrollbar" aria-busy={result.isLoading}>
        {result.isLoading ? (
          ...
          <div className="p-4 text-center" role="alert">
            <p className="text-xs text-semantic-error">{t('loadOrdersError')}</p>
            <Button className="mt-2" variant="outline" size="xs" onClick={() => result.refetch()}>{t('retryLoadBtn')}</Button>
          </div>
        ) : orders.length === 0 ? (
          <p className="p-4 text-center text-xs text-muted">{t('noMatchingOrders')}</p>
        ) : orders.map((order) => (
          <button
          ...
          >
            <span className="min-w-0">
              ...
              <span className="block text-[10px] text-muted">{new Date(order.syncedAt).toLocaleString(locale, { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit' })}</span>
            </span>
      ...
      {(result.data?.meta.totalPages ?? 1) > 1 && (
        <div className="flex items-center justify-between border-t border-hairline p-2">
          <Button variant="ghost" size="xs" disabled={page <= 1} onClick={() => setPage((current) => current - 1)}>{t('previousPage')}</Button>
          <span className="text-[10px] text-muted">{page}/{result.data?.meta.totalPages}</span>
          <Button variant="ghost" size="xs" disabled={page >= (result.data?.meta.totalPages ?? 1)} onClick={() => setPage((current) => current + 1)}>{t('nextPage')}</Button>
        </div>
      )}
    </aside>
  );
